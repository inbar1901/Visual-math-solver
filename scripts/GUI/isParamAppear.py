"""
This file is used in te GUI,for paramteric equation
...
the output is  full image with the parameter eq the solution
"""
import os
import random
import numpy as np
...
def isParamAppear (equation, sol:str, img,param):
    ...
    solution = str(sol[0]).replace(" ","")
    # creating parameter image for answer
    param_BB = digits_bb[ str(equation).find(str(param))][1]
    param_cropImg = img[:, param_BB[0]:param_BB[1]]
    param_random_img, param_group = matchGroup(param, param_cropImg, run_number)

    # creating eq sign image for answer
    eq_img_path = r'/home/inbarnoa/PycharmProjects/MathSolver/DataSet/DataSetNumbers/extracted_images/=/=_13599.jpg'
    eq_image = cv2.imread(eq_img_path, cv2.IMREAD_GRAYSCALE)
    ret, thresh1 = cv2.threshold(eq_image, 200, 255, cv2.THRESH_BINARY_INV)
    kernel = np.ones((3, 3), np.uint8)
    eq_img_erosion = cv2.dilate(thresh1, kernel, iterations=1)
    eq_img = scipy.ndimage.gaussian_filter(eq_img_erosion, 1)

    # creating result image
    current_width = param_random_img.shape[1] + eq_img.shape[1] + 3
    pic_height = param_random_img.shape[0] +15
    pic_width = (len(solution) * (45+mean_spacing_x+5)) + current_width

    pic = np.zeros((pic_height,pic_width ), dtype=np.uint8)
    pic[5:param_random_img.shape[0]+5,0:param_random_img.shape[1]] = param_random_img
    pic [6:param_random_img.shape[0]+6,param_random_img.shape[1] +3:current_width] = eq_img
    plt.imshow(pic)
    special_fig = ['I', '-', '+', '/', '.','*','s','q','r','t','(',')']
    # generating the solution image only for numbers
    for idx,char in enumerate(solution):
        if char in special_fig:
            if char == '-':
                random_img_path = r'/home/inbarnoa/PycharmProjects/MathSolver/DataSet/DataSetNumbers/extracted_images/-/-_1350.jpg'
            elif char == '+':
                random_img_path = r'/home/inbarnoa/PycharmProjects/MathSolver/DataSet/DataSetNumbers/extracted_images/+/+_1055.jpg'
            elif char == '/':
                random_img_path = r'/home/inbarnoa/PycharmProjects/MathSolver/DataSet/DataSetNumbers/extracted_images/\div/div_17994.jpg'
            elif char == '.':
                random_img_path = r'/home/inbarnoa/PycharmProjects/MathSolver/DataSet/DataSetNumbers/extracted_images/cdot/Iimage511.jpg.bmp.bmp'
            elif char == '*':
                random_img_path = r'/home/inbarnoa/PycharmProjects/MathSolver/DataSet/DataSetNumbers/extracted_images/cdot/I6669.jpg.bmp'
            elif char == 's':
                random_img_path = r'/home/inbarnoa/PycharmProjects/MathSolver/DataSet/DataSetNumbers/extracted_images/S/exp26751.jpg'
            elif char == 'q':
                random_img_path = r'/home/inbarnoa/PycharmProjects/MathSolver/DataSet/DataSetNumbers/extracted_images/q/exp28417.jpg'
            elif char == 'r':
                random_img_path = r'/home/inbarnoa/PycharmProjects/MathSolver/DataSet/DataSetNumbers/extracted_images/R/exp100801.jpg'
            elif char == 't':
                random_img_path = r'/home/inbarnoa/PycharmProjects/MathSolver/DataSet/DataSetNumbers/extracted_images/T/exp4152.jpg'
            elif char == '(':
                random_img_path = r'/home/inbarnoa/PycharmProjects/MathSolver/DataSet/DataSetNumbers/extracted_images/(/(_749.jpg'
            elif char == ')':
                random_img_path = r'/home/inbarnoa/PycharmProjects/MathSolver/DataSet/DataSetNumbers/extracted_images/)/)_827.jpg'
            else: # this is i
                random_img_path = r'/home/inbarnoa/PycharmProjects/MathSolver/DataSet/DataSetNumbers/extracted_images/i/exp6602.jpg'
...
# path = r'/home/inbarnoa/PycharmProjects/MathSolver/equationParameter/A36_0.bmp'
# # path = r'77.jpg'
# img = cv2.imread(path,cv2.IMREAD_GRAYSCALE)
# isParamAppear("a-3=", "3", img,"a")