...
THis is the main module for training the HMER network
notice to update the dataset, output name, and graph and log name
'''
import torch
import math
...
datasets=['./offline-train_4.pkl','./train_caption_4.txt']
valid_datasets=['./offline-val_4.pkl', './val_caption_4.txt']
...
for epoch in range(120):
    ...
    for step,(x,y) in enumerate(train_loader):
        ...
        if step % 20 == 19:
            ...
            print('epoch is %d, lr rate is %.7f, te is %.3f, batch_size is %d, loading for %.3f%%, running_loss is %f' %(epoch,lr_rate,teacher_forcing_ratio, batch_size,pre,running_loss))
            # with open("training_data/running_loss_%.5f_pre_GN_te05_d02_all.txt" %(lr_rate),"a") as f:
            #     f.write("%s\n"%(str(running_loss)))
            running_loss = 0

    loss_all_out = whole_loss / len_train
    total_train_loss.append(loss_all_out)
    ...
    if (sacc > exprate):
        ...
        print('encoder_lr%.5f_GN_te1_d05_SGD_bs6_mask_conv_bn_b_xavier_13.pkl' %(lr_rate))
        torch.save(encoder.state_dict(), 'model/encoder_lr%.5f_GN_te1_d05_SGD_bs6_mask_conv_bn_b_xavier_13.pkl'%(lr_rate))
        torch.save(attn_decoder1.state_dict(), 'model/attn_decoder_lr%.5f_GN_te1_d05_SGD_bs6_mask_conv_bn_b_xavier_13.pkl'%(lr_rate))
        print("done")
        flag = 0
    else:
        flag = flag+1
    ...
    ax.plot(np.arange(len(total_train_loss)), total_train_loss, label='train loss')
    ax.set_title('train loss')
    ax.set_xlabel('epoch')
    ax.set_ylabel('loss')
    ax.legend()
    plt.savefig("trainLoss13.png")

    fig, ax = plt.subplots(figsize=(5, 5))
    ax.plot(np.arange(len(total_wer)), total_wer, label='WER')
    ax.set_title('test WER')
    ax.set_xlabel('epoch')
    ax.set_ylabel('WER')
    ax.legend()
    plt.savefig("WER13.png")

    fig, ax = plt.subplots(figsize=(5, 5))
    ax.plot(np.arange(len(total_succ)), total_succ, label='succ')
    ax.set_title('test succ')
    ax.set_xlabel('epoch')
    ax.set_ylabel('succ')
    ax.legend()
    plt.savefig("succ13.png")
